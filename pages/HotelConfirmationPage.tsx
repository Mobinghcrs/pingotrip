...
const HotelConfirmationPage: React.FC = () => {
    ...
    if (!bookingContext || !bookingContext.selectedHotel) {
        ...
        <div className="bg-slate-50 dark:bg-slate-950 min-h-screen">
            <style>{`
                @media print {
                    ...
                    #voucher-to-print { position: absolute; left: 50%; top: 2rem; transform: translateX(-50%); width: 100%; max-width: 28rem; }
            ...
            <header className="bg-sky-900 text-white p-4 rounded-b-2xl no-print">
                ...
                <p className="text-center text-sm opacity-90">{t('hotelBookingSuccessSub')}</p>
            </header>

            <main className="p-4">
                <HotelVoucher 
                    hotel={selectedHotel}
                    guestName={t('pishgamanTech')} // Mock guest name
                    checkInDate="۱۴۰۳/۰۸/۱۰" // Mock date
                    checkOutDate="۱۴۰۳/۰۸/۱۲" // Mock date
                />
                ...
                <div className="mt-6 flex flex-col gap-3 no-print">
                    <button onClick={handleDownload} className="w-full flex items-center justify-center gap-2 bg-white dark:bg-slate-800 border-2 border-cyan-500 text-cyan-600 dark:text-cyan-400 font-bold py-3 px-4 rounded-lg hover:bg-cyan-50 dark:hover:bg-sky-900 transition-colors">
                        <ArrowDownCircleIcon className="w-5 h-5"/> {t('downloadVoucher')}
                    </button>
