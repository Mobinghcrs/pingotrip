...
const CurrencyDepositAdminPage = () => {
    const { t, language } = useTranslation();
    const [allBookings, setAllBookings] = useState<Purchase[]>(MOCK_PURCHASES);
    const [isSidebarOpen, setIsSidebarOpen] = useState(true);
    
    const bookings = allBookings.filter(b => b.type === 'currencyDeposit');
    ...
    return (
        <div className="relative min-h-screen bg-slate-100">
            <AdminSidebar 
                ...
                activeTab="currency-deposit-admin"
            />
            <div className="md:mr-64">
                <AdminHeader onMenuClick={() => setIsSidebarOpen(true)} />
                <main className="flex-1 p-4 sm:p-6 lg:p-8">
                    <h2 className="text-2xl font-bold text-slate-800 mb-6">{t('currencyDepositAdmin')}</h2>
...
export default CurrencyDepositAdminPage;